...
public class HerdSimCore : MonoBehaviour
{
	...
	public bool _scared;
	//If this is scared it will run and flee away from _scareOf
	...
	public void Start() {
		...
		_thisTR = transform;
		_animation = _model.GetComponent<Animation>();
		...
		if (!_animator) {
			_animation[this._animIdle].speed = _animIdleSpeed;
			_animation[this._animDead].speed = _animDeadSpeed;
			_animation[this._animSleep].speed = _animSleepSpeed;
		} else {
			_animator.speed = _animIdleSpeed;
		}
	...
	public void Death() {
		if (!_dead) {
			...
			if (!_animator) {
				_animation.CrossFade(_animDead, .1f);
				_animation[this._animDead].speed = 1.0f;
			} else {
				_animator.speed = 1.0f;
				_animator.CrossFade(_animDead, .1f);
			}
