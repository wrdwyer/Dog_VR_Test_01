            ...
            using (new EditorGUI.DisabledScope(acousticMap.Computing))
            {
                ...
                if (showMappingConfig)
                {
                    ...
                    using (new EditorGUI.DisabledGroupScope(!customPointsEnabled))
                    {
                        ...
                        {
                            EditorGUILayout.PrefixLabel(" ");
                            // Toggle enabling of editing points in the scene view.
                            string enableToggleLabel = freeEditEnabled ? "Exit Free Edit Mode" : "Enter Free Edit Mode";
                            if (GUILayout.Button(new GUIContent(enableToggleLabel, "Toggle editing of the points in the scene view.")))
