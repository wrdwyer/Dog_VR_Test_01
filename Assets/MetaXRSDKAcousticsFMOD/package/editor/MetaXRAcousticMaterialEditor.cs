...
internal class MetaXRAcousticMaterialEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        bool hadCustomData0 = mat0.HasCustomData;
#else
        ...
        foreach (Object t in targets)
        {
            ...
            if (mat.Properties != props0)
                propsIsMixed = true;
#if OVR_INTERNAL_CODE
            if (mat.HasCustomData != hadCustomData0)
                customDataIsMixed = true;
#endif
        }
...
#if OVR_INTERNAL_CODE
        bool hasCustomData = EditorGUI.ToggleLeft(r, "Override Properties", hadCustomData0);
        if (hasCustomData != hadCustomData0)
            customDataIsMixed = false;
#endif
        ...
        else if (isInvalid)
        {
            char warning = '\u26A0';
#if OVR_INTERNAL_CODE
            EditorGUILayout.LabelField($"{warning} No material properties specified, you must select a preset or override!", new GUIStyle
#else
...
#if OVR_INTERNAL_CODE
                if (!customDataIsMixed)
                {
                    if (hadCustomData0)
                        mat.customData.color = newColor;

                    mat.HasCustomData = hasCustomData;

                    if (shouldCopyFromPreset)
                        mat.customData.Clone(props.Data);

                    ApplyChanges(mat, newPreset);
                }
#endif
            }
        }
...
#if OVR_INTERNAL_CODE
    internal void ApplyChanges(MetaXRAcousticMaterial mat, BuiltinPreset newPreset)
    {
        if (newPreset != BuiltinPreset.Custom)
            Undo.SetCurrentGroupName($"Apply {newPreset} preset");

        string groupName = Undo.GetCurrentGroupName();
        Undo.RegisterCompleteObjectUndo(mat, groupName);

        if (groupName == MetaXRAudioSpectrumEditor.pointAddedGroupName)
            Undo.CollapseUndoOperations(Undo.GetCurrentGroup() - 1);

        Meta.XR.Acoustics.MaterialData data = mat.customData;
        if (newPreset != BuiltinPreset.Custom)
            SetPreset(newPreset, ref mat.customData);
