...
public class MetaXRAcousticGeometryEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        newEnableDiffraction = EditorGUILayout.Toggle(new GUIContent("Enable Diffraction", "Build mesh to support real time diffraction"), newEnableDiffraction);
#endif
        ...
        if (showAdvancedControls)
        {
...
#if OVR_INTERNAL_CODE
            Separator();
            using (new EditorGUI.DisabledGroupScope(!newEnableDiffraction))
            {
                EditorGUILayout.LabelField("Diffraction", EditorStyles.boldLabel);
                newMinDiffractionEdgeAngle = EditorGUILayout.Slider(new GUIContent("Min Edge Angle", "Minimum required angle between surfaces at an edge for it to produce diffraction"), newMinDiffractionEdgeAngle, 0.0f, 90.0f);
                newMinDiffractionEdgeLength = EditorGUILayout.Slider(new GUIContent("Min Edge Length", "Minimum required length of an edge for it to produce diffraction"), newMinDiffractionEdgeLength, 0.0f, 1.0f);
                newFlagLength = EditorGUILayout.Slider(new GUIContent("Flag Length", "The maximum length that a diffraction flag can extend out from an edge. Longer flags produce more robust diffraction, but also reduce performance."), newFlagLength, 0.0f, 1.0f);
            }
#endif
            Separator();
            ...
            {
#if OVR_INTERNAL_CODE
                newFileEnabled = EditorGUILayout.Toggle(new GUIContent("File Enabled", "If set, the serialized mesh file is used as the mesh data source"), newFileEnabled);
                using (new EditorGUI.DisabledGroupScope(!newFileEnabled))
                {
#endif
